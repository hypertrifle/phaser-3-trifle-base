...
export default class GameData extends Phaser.Plugins.BasePlugin {
  ...
  public init() {
    ...
    this.loadData([
      this.game.cache.json.get("settings"),
      this.game.cache.json.get("content")
    ]);
  ...
  private loadData(contentJSONObject: any | Array<any>) {
    ...
    let contentJSONObjects = Array.isArray(contentJSONObject)
      ? contentJSONObject.reverse()
      : [contentJSONObject];
        ...
        if (data[j] === undefined) {
          ...
          for (let k in data[j]) {
            if (contentJSONObject[i][j][k] !== undefined) {
              console.warn(
                "json file conflict, data for '" +
                  j +
                  "." +
                  k +
                  "' has multiple definitions, will use: " +
                  contentJSONObject[i][j][k]
              );
            }
          }
    ...
    if (path !== undefined && path.trim() !== "") {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (obb === undefined) {
          ...
          console.warn(
            "game.data.get, tried to request key of: " +
              path +
              " but failed at getting: ",
            errObj
          );
          return null;
        }
