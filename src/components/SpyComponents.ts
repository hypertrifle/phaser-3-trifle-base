...
import { SpyTimeline, Action } from "../models/SpyModels";
...
export class SpyMap extends BaseComponent {
    ...
    setupEvents() {
        ...
        this.scene.events.on("state:member", this.disableButtons);
        this.scene.events.on("game.time-change", this.updatePositions);
    }
    ...
    buttonClick() {
        //find associated Interactable tile model and update the game state to reflect the choise.
        this.scene.events.emit("state:member");
    }
    ...
    highlightButtons(model:SpyTimeline) {

    }

    updatePositions(model:SpyTimeline){
